...
test('daily discovery keeps preferences, rotates and targets a group', async ({ page }) => {
  ...
  await drawer.getByRole('checkbox', { name: '经验', exact: true }).uncheck();
  await drawer.getByRole('textbox', { name: '兴趣关键词', exact: true }).fill('跑步');
  await drawer.getByRole('button', { name: '保存偏好' }).click();
  await expect(drawer).toBeHidden();
  await page.reload();
  await expect(discovery.getByRole('link', { name: '去看看' })).toHaveAttribute(
    'href',
    /interest-groups\?group=/,
  );
  const title = await discovery.locator('h4').innerText();
  await discovery.getByRole('link', { name: '去看看' }).click();
  await expect(page).toHaveURL(/interest-groups\?group=/);
  await expect(page.getByRole('heading', { name: title, exact: true })).toBeVisible();
  await expect(page.locator('.record-card')).toHaveCount(1);
  await page.getByRole('button', { name: '查看全部趣缘群体' }).click();
  await expect(page).toHaveURL(/interest-groups$/);
});
