...
- 已实现工作台、经验库、信息与咨询、社群与俱乐部、活动、联络资源、体育代表队、财务治理、权限与模块管理九个页面；
...
| 社群与俱乐部 | 俱乐部目录、成员协作和活动关联 | 独立专题组或认领团队 |
...
本地默认使用 `memory + demo`，可在页面中切换固定演示身份；进程重启后内存数据会清空。MySQL 联调、环境变量和常见问题请按[本地开发与完整预览](./docs/local-development.md)操作。
