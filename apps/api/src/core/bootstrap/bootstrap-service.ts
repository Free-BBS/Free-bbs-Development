...
export interface BootstrapExecutionLock {
...
export async function bootstrapPlatform(
  ...
  executionLock: BootstrapExecutionLock,
  ...
  return executionLock.withLock(() =>
    store.transaction(async (transactionStore) => {
      ...
      if (
        !input.recovery &&
        superAdminAssignments.some((assignment) =>
          isEffectiveActiveSuperAdmin(assignment, input.now),
        )
      ) {
